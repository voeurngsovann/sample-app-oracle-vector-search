...
.login-form-title {
    font-family: var(--font-display) !important;
    font-size: 1.75rem; font-weight: 700; color: var(--text); margin-bottom: 6px;
}
.login-form-sub { font-size: .82rem; color: var(--text3); margin-bottom: 36px; }
.fl {
...
def render_login():
    ...
    /* float form panel fixed to right */
    [data-testid="stAppViewContainer"] > section.main > div.block-container > div:last-child {
    ...
    # Left hero panel (pure HTML — no Streamlit widgets here)
    st.markdown("""
    <div class="login-split" style="pointer-events:none;">
      <div class="login-left">
        ...
        <div class="login-left-content">
          ...
          <div class="login-headline">
            Semantic<br><span>Vector</span><br>Intelligence
          </div>
          <div class="login-tagline">
            Search your knowledge base using meaning, not keywords.
            Powered by in-database ONNX embeddings and HNSW indexing.
          </div>
          <div class="login-pills">
            <span class="login-pill">ONNX Embeddings</span>
            <span class="login-pill">HNSW Index</span>
            <span class="login-pill">RAG · Ollama</span>
            <span class="login-pill">Oracle 26ai</span>
          </div>
          <div class="login-stats">
            <div class="login-stat">
              <div class="login-stat-val">384</div>
              <div class="login-stat-lbl">Vector Dims</div>
            </div>
            <div class="login-stat">
              <div class="login-stat-val">HNSW</div>
              <div class="login-stat-lbl">Index Type</div>
            </div>
            <div class="login-stat">
              <div class="login-stat-val">ANN</div>
              <div class="login-stat-lbl">Search Mode</div>
            </div>
          </div>
        </div>
      </div>
    ...
    # Right panel — Streamlit form (positioned fixed via CSS above)
    st.markdown('<div class="login-form-title syne">Welcome back</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="login-form-author">Made by Voeurng Sovann</div>', unsafe_allow_html=True)
        ...
        if not username or not password:
            ...
            try:
                ...
                if user:
                    ...
                    logger.info("UI login success: %s", username)
                    st.rerun()
                else:
                    st.session_state.login_error = "Invalid username or password."
                    logger.warning("UI login failed: %s", username)
                    st.rerun()
            except Exception as e:
                st.session_state.login_error = "Connection error — check DB settings."
...
def render_app():
    ...
    with st.sidebar:
        ...
        from rag import is_rag_available, list_models
        if mode == "rag":
            if is_rag_available():
                models      = list_models()
                default_mdl = os.environ.get("OLLAMA_MODEL", "gemma4:e4b")
                ...
                st.warning(
                    f"Ollama not reachable at\n`{os.environ.get('OLLAMA_BASE_URL','http://localhost:11434/api')}`\n\nStart: `ollama serve`",
                    icon="⚠️",
                )
                selected_model = os.environ.get("OLLAMA_MODEL", "gemma4:e4b")
        else:
            selected_model = None
