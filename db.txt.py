...
def ensure_question_cache_table() -> None:
    """Create question_cache table for caching user queries and answers."""
    cache_table = "QUESTION_CACHE"
    seq_name = f"SEQ_{cache_table}_ID"

    with get_conn() as conn:
        cur = conn.cursor()

        if not _table_exists(cur, cache_table):
            # Create sequence
            if not _sequence_exists(cur, seq_name):
                cur.execute(
                    f"CREATE SEQUENCE {cfg.schema}.{seq_name} "
                    f"START WITH 1 INCREMENT BY 1 NOCACHE NOCYCLE"
                )
                logger.info("Created sequence %s.%s", cfg.schema, seq_name)

            cur.execute(f"""
                CREATE TABLE {cfg.schema}.{cache_table} (
                    id              NUMBER          DEFAULT {cfg.schema}.{seq_name}.NEXTVAL
                                                    NOT NULL
                                                    CONSTRAINT PK_{cache_table}_ID PRIMARY KEY,
            ...
            cur.execute(f"""
                CREATE INDEX IDX_{cache_table}_USER_Q
                ON {cfg.schema}.{cache_table}(user_id, question)
            """)
            logger.info("Created index IDX_%s_USER_Q", cache_table)
...
#  Question cache
# ─────────────────────────────────────────────────────────
def cache_answer(user_id: int, question: str, answer: str, response_time: float) -> None:
    """Store a question and answer in cache."""
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            f"""
            INSERT INTO {cfg.schema}.QUESTION_CACHE
            (user_id, question, answer, response_time)
            VALUES (:user_id, :question, :answer, :response_time)
            """,
            {"user_id": user_id, "question": question, "answer": answer, "response_time": response_time},
        )
        logger.debug("Cached answer for user_id=%d  question=%s", user_id, question[:50])


def get_cached_answer(user_id: int, question: str) -> dict | None:
    """Retrieve cached answer if exists (case-insensitive match)."""
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            f"""
            SELECT answer, response_time
            FROM {cfg.schema}.QUESTION_CACHE
            WHERE user_id = :user_id AND LOWER(question) = LOWER(:question)
            """,
            {"user_id": user_id, "question": question},
        )
        row = cur.fetchone()
        if row:
            return {"answer": row[0], "response_time": row[1]}
        return None


def clear_cache_for_user(user_id: int) -> None:
    """Clear all cached answers for a user (e.g., after document delete)."""
    with get_conn() as conn:
        cur = conn.cursor()
        cur.execute(
            f"DELETE FROM {cfg.schema}.QUESTION_CACHE WHERE user_id = :user_id",
            {"user_id": user_id},
        )
        logger.info("Cleared cache for user_id=%d  rows=%d", user_id, cur.rowcount)
